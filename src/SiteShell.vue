...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('funeraryObjects.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('funeraryObjects.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('funeraryObjects.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
